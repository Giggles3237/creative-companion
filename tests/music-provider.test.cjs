...
function provider(responses){
 ...
 return {generate:exports.generate,testElevenMusicConnection:exports.testElevenMusicConnection,calls,logs};
}
...
test('admin music diagnostic reports audio bytes on success',async()=>{
 const app=provider([new Response(new Uint8Array([73,68,51,4]),{headers:{'content-type':'audio/mpeg'}})]);
 const result=await app.testElevenMusicConnection();
